...
// MEMBER DASHBOARD
const getMemberOverview = async (email: string) => {
...
const getUpcomingEvents = async (email: string) => {
  const now = new Date();
...
const getMyClubs = async (email: string) => {
  const memberships = await Membership.find({ email, status: "active" });
  const clubIds = memberships.map((m) => m.clubId);
  if (!clubIds.length) return [];
...
const getMyEvents = async (email: string) => {
...
// MANAGER DASHBOARD
const getManagerOverview = async (email: string) => {
...
const getManagerClubs = async (email: string) => {
  const clubs = await Club.find({ managerEmail: email, status: "approved" });
  return clubs;
};

const getClubMembers = async (email: string) => {
  const clubs = await Club.find({ managerEmail: email, status: "approved" });
  const clubIds = clubs.map((c) => c._id.toString());
...
const getManagerEvents = async (email: string) => {
  const clubs = await Club.find({ managerEmail: email, status: "approved" });
  const clubIds = clubs.map((c) => c._id.toString());
...
const getEventRegisteredMembers = async (email: string) => {
  const clubs = await Club.find({ managerEmail: email, status: "approved" });
  const clubIds = clubs.map((c) => c._id.toString());
  const events = await Event.find({ clubId: { $in: clubIds } });
